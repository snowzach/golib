...
func New(c *Config) (*sqlx.DB, error) {
	...
	if c.AutoCreate {
		...
		createConnConfig, err := pgx.ParseConfig(dbCreateCreds + "dbname=postgres " + connStr.String())
		if err != nil {
		...
		c.Logger.Printf("Checking for database: %s", c.Database)
		var one int
		if err := db.Get(&one, `SELECT 1 from pg_database WHERE datname=$1`, c.Database); err == sql.ErrNoRows {
			c.Logger.Printf("Creating database: %s", c.Database)
		...
		createConnConfig.Database = c.Database

		db, err = sqlx.Connect("pgx", stdlib.RegisterConnConfig(createConnConfig))
		if err != nil {
			return nil, fmt.Errorf("could not connect to database after checking existance error: %v", err)
		}

		c.Logger.Printf("Checking for schema: %s", c.Schema)
		if err := db.Get(&one, `SELECT 1 from information_schema.schemata WHERE schema_name=$1`, c.Schema); err == sql.ErrNoRows {
			c.Logger.Printf("Creating schema: %s", c.Schema)
			_, err = db.Exec(`CREATE SCHEMA "` + c.Schema + `"`)
			if err != nil {
			...
			// Some other error besides does not exist
			return nil, fmt.Errorf("could not check for schema: %w", err)
		}
	...
	// Connect to database
	connStr.WriteString(fmt.Sprintf("dbname=%s ", c.Database))
